...
const Product = () => {
  ...
  const handleAddToCart = () => {
    if (prod && localStock >= quantity) {
      const mainImage =
        (prod.images?.[selectedImage]?.imageUrl &&
          toAbsoluteImage(prod.images[selectedImage].imageUrl)) ||
        (prod.images?.[0]?.imageUrl &&
          toAbsoluteImage(prod.images[0].imageUrl)) ||
  ...
  const relatedProducts = useMemo(() => {
    if (!prod || !Array.isArray(allProducts) || allProducts.length === 0)
      return [];
  ...
  if (status === "failed") {
    return (
      <div className="error-message">{error || "Failed to load product"}</div>
    );
  }
  if (!prod) {
  ...
  return (
    <div className="product-page">
      <div className="breadcrumb">
        ...
        <Link to={`/category/${prod.category?.id}`}>
          {prod.category?.name || "Category"}
        </Link>
        <FaChevronRight className="chevron" />
        <Link to={`/subcategory/${prod.subcategory?.id}`}>
          {prod.subcategory?.name || "Subcategory"}
        </Link>
        <FaChevronRight className="chevron" />
      ...
      <div className="product-container">
        ...
        <div className="gallery-container">
          <div className="thumbnail-strip-pc">
        ...
        <div className="details-container">
          ...
          <div className="pricing-container">
            ...
            {hasDiscount && (
              <div className="savings-badge">
                Save $
                {(Number(prod.price) - Number(prod.discountPrice)).toFixed(2)}
              </div>
            )}
          </div>
        ...
        <div className="related-section">
          ...
          <div className="related-grid">
            {relatedProducts.map((rp) => {
              const rpImage =
                toAbsoluteImage(rp.images?.[0]?.imageUrl) ||
              ...
              return (
                <div className="related-card" key={rp.id}>
                  ...
                  <div className="related-info">
                    <Link to={`/product/${rp.id}`} className="related-name">
                      {rp.name}
                    </Link>
                    <div className="related-price">
                      ${Number(rp.discountPrice || rp.price).toFixed(2)}
                    </div>
                    <button
                      className="add-btn"
                      onClick={() =>
