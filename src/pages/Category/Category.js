...
const Category = () => {
  ...
  // 🔹 Get current category name from Redux using categoryId
  ...
  // 🔹 Fallback-safe dynamic title
  const categoryTitle = category?.name || "Shop by Category";

  const { subcategories, status: subcategoriesStatus } = useSelector(
  ...
  const getSubcategoryImage = useCallback((subcategory) => {
    ...
    if (subcategory.images && subcategory.images.length > 0) {
      // Find the primary image or use the first one
      const primaryImage =
        subcategory.images.find((img) => img.isPrimary) ||
  ...
  const isLoading =
    brandsStatus === "loading" || subcategoriesStatus === "loading";
  const hasError =
    brandsStatus === "failed" || subcategoriesStatus === "failed";
          ...
          {isLoading ? (
            ...
            Array.from({ length: 6 }).map((_, index) => (
              <div
                ...
                className="subcategory-card-skeleton"
              >
